...
<html>
<head>
    <title><fmt:message key="label.admin.countries.create" bundle="${ rb }" /></title>
    <meta charset="utf-8">
...
<section>
    <div class="container-fluid">
        <span style="text-align: center;"><h1><fmt:message key="label.admin.countries.create" bundle="${ rb }" /></h1></span>
        <ctg:before-grid />
        <form id="create-trip-form" method="post" action="travel" enctype="multipart/form-data" accept-charset="utf-8">
            <input type="hidden" name="command" value="create_country" />
            <div class="row">
                <div class="form-group col-sm-6 col-sm-offset-3">
                    <label for="name"><fmt:message key="label.admin.create-tour.tour.name" bundle="${ rb }" /></label>
                    <input type="text" class="form-control" id="name" name="name" max="45" required>
                </div>
            </div>
            <div class="row">
                <div class="col-sm-2 col-sm-offset-5">
                    <c:choose>
                        <c:when test="${ not empty errorCreateCountryPassMessage }">
                            <br/>
                            <span style="color: #ff0000;">${errorCreateCountryPassMessage}</span>
                            <br/>
                            <br/>
                        </c:when>
                        <c:otherwise>
