                ...
                <div class="form-group col-sm-6 col-sm-offset-3">
                    ...
                    <select class="form-control" id="destination-city" name="destination-city">
                        <c:forEach var="city" items="${cities}">
                            <c:choose>
                                <c:when test="${ city.id == (vacation.cities[0].id) }"><option selected value=<c:out value="${ city.id }" />><c:out value="${ city.name }" /> (<c:out value="${ city.country.nameCountry }" />)</option></c:when>
